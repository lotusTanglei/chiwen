...
你支持 6 个命令：`init`、`sync`、`onboard`、`status`、`ask`、`guide`。
...
触发条件：用户提出关于项目的问题，或用户说 "ask"。

**核心原则：完全基于 `.docs/` 中的文档回答，不读取代码，不猜测。**

流程：

1. **理解用户问题**
   - 识别问题涉及的领域（架构、能力、模块、流程等）

2. **定位相关文档**
   - 架构相关 → 读取 `1_ARCHITECTURE.md`
   - 能力相关 → 读取 `2_CAPABILITIES.md`
   - 特定模块相关 → 读取 `modules/{module_name}/README.md`（递归查找子模块）
   - 决策相关 → 读取 `4_DECISIONS.md`
   - 路线图相关 → 读取 `3_ROADMAP.md`
   - 不确定 → 先读取 `0_INDEX.md` 定位，再读取具体文档

3. **基于文档内容回答**
   - 只使用文档中的信息，不读取源代码
   - 如果文档中没有相关信息，明确告知用户"文档中未记录此信息，建议执行 sync 更新文档或手动补充"
   - 引用具体文档路径，方便用户查阅原文

4. **如果文档不足以回答**
   - 告知用户哪些文档缺失或信息不足
   - 建议执行 `init`（文档不存在）或 `sync`（文档可能过时）

---

### guide — 功能实现指引

触发条件：用户描述一个要实现的功能需求，或用户说 "guide"。

**核心原则：告诉 LLM 编码助手"改哪里、怎么改、参考谁"，不替它写代码。**

流程：

1. **理解需求** — 识别涉及的业务域
2. **定位模块** — 读取 `1_ARCHITECTURE.md` + 相关 `modules/*/README.md`
3. **生成指引** — 输出三部分：
...
规则：
- 只提供方向和约束，不写具体代码、类名、方法签名
- 具体实现由 LLM 编码助手根据指引自行决定
- 如果模块文档中没有「修改指引」章节，提示用户补充
...
**`## 修改指引`**
- 描述在这个模块中新增功能或修改代码时应遵循的约定
- 包含以下子章节（按需）：
  - `### 新增功能时` — 应该在哪些目录下创建哪些文件，遵循什么命名约定
  - `### 约定与规范` — 代码风格、注解使用、异常处理等团队约定
  - `### 相关配置` — 需要同步修改的配置文件（路由、权限、数据库等）
- 这部分是"设计辅助"命令的核心数据源，LLM 会基于此生成精准的实现提示词
...
如果一个模块有子模块（`Module.children` 非空），则：
- 父模块的 README.md 在「源文件与职责」中列出子模块目录（而非子模块内的文件）
- 每个子模块有自己的 `README.md`，递归遵循相同的章节结构
- 例如 `modules/bixi-common/README.md` 概述整个 common 包，`modules/bixi-common/bixi-common-core/README.md` 详述 core 子模块
