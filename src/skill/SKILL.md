...
8. **遍历 `skipped_for_llm` 中的模块文档**
   - 筛选出 `modules/*.md` 格式的文件名
   - 对每个模块文档，从 `CodeReaderOutput` 中找到对应源文件的信息（public_api、文件路径、所属模块）

9. **撰写每个模块文档**
   - 严格遵循「Module_Doc 撰写指引」章节的格式规范
   - 基于对应源文件的代码内容和 `CodeReaderOutput` 撰写
   - 将撰写结果写入 `.docs/modules/{filename}.md`
...
**标题和源文件标注**
- 一级标题使用模块的人类可读名称（如「代码扫描引擎」而非 `code_reader`）
- 紧跟一行引用标注源文件路径

**`## 职责`**
- 一段话描述这个模块做什么，面向团队成员阅读
- 不要罗列函数名，用自然语言描述
...
**`## 依赖关系`**
- 列出该模块依赖的其他模块（import 了哪些内部模块）
- 列出哪些模块依赖了它（被谁 import）
- 使用简单列表格式
