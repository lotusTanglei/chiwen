...
**核心原则：基于 `.docs/` 中的架构、模块文档和修改指引，生成精准的实现思路和提示词。**
...
2. **定位相关模块**
   - 读取 `1_ARCHITECTURE.md` 了解整体架构和模块职责映射
   - 根据需求定位到具体模块（可能涉及多个模块）
   - 读取相关模块的 `modules/{module_name}/README.md`
   - 如果有子模块，递归读取子模块文档
...
4. **生成实现指引**
   - 输出结构化的实现思路，包含：
     ...
     - **实现步骤**：按顺序列出具体的开发步骤
     - **代码约定**：该模块的编码规范和注意事项
     - **关联修改**：需要同步修改的配置、路由、权限等
     - **参考实现**：指向文档中提到的类似功能作为参考
   - 如果需要，生成可以直接交给 AI 编码助手的提示词
